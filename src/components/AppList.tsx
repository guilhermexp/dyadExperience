...
export function AppList({ show }: { show?: boolean }) {
  ...
  return (
    <>
      ...
      >
        <SidebarGroupLabel className="text-muted-foreground dark:text-white/70 px-4 mb-3">Your Apps</SidebarGroupLabel>
        <SidebarGroupContent>
          <div className="flex flex-col space-y-2 px-3">
            <Button
              onClick={handleNewApp}
              variant="ghost"
              className="flex items-center justify-start gap-2 h-9 px-3 hover:bg-muted dark:hover:bg-white/10 text-foreground dark:text-white/90 hover:text-foreground dark:hover:text-white transition-colors rounded-lg"
            >
            ...
            {loading ? (
              ...
              <div className="py-2 px-4 text-sm text-red-500 dark:text-red-400">
                Error loading apps
              </div>
            ) : apps.length === 0 ? (
              <div className="py-2 px-4 text-sm text-muted-foreground">
                ...
                {apps.map((app) => (
                  <div key={app.id}>
                    <Button
                      ...
                      className={`justify-start w-full text-left px-3 py-2 hover:bg-muted dark:hover:bg-white/10 transition-colors rounded-lg ${
